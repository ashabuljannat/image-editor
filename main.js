...
const applyFilter = () => {
  ...
  previewImg.style.filter = `brightness(${brightness}%) saturate(${saturation}%) invert(${inversion}%) grayscale(${grayscale}%) blur(${blur}px) contrast(${contrast}%)  sepia(${sepia}%)`;
};
...
const saveImage = () => {
  ...
  blur(${blur}px) contrast(${contrast}%) sepia(${sepia}%)`;
